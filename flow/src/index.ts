...
export default {
  async fetch(
    ...
    try {
      ...
      return new Response("unauthorized or server error", {
...
async function handlePullRequestSynchronize({ octokit, payload }: any) {
  const owner = payload.repository.owner.login;
  const repo = payload.repository.name;
  const prNumber = payload.pull_request.number;

  console.log(`new commit pushed to pr #${prNumber}. re-evaluating...`);
  await processPullRequest(octokit, owner, repo, prNumber);
}
...
async function processPullRequest(
  ...
  repo: string,
  prNumber: number,
  options = { isApprovedOverride: false },
) {
  try {
    ...
    if (options.isApprovedOverride) {
      console.log(
        `pr #${prNumber} was approved. overriding blocks and attempting merge.`,
      );
      await removeLabels(octokit, owner, repo, prNumber, labels, [
        "blocked",
        "wfr",
      ]);
      await attemptMerge(
    ...
    if (hasWfr) {
      console.log(
        `pr #${prNumber} has 'wfr' label. skipping auto-merge until reviewed.`,
      );
      return;
    }
    ...
    if (conclusion === "success") {
      ...
      console.log(
        `pr #${prNumber} checks are still '${conclusion}'. waiting for completion.`,
      );
    }
  } catch (error: any) {
...
async function attemptMerge(
  ...
  try {
    ...
    console.log(
      `successfully deleted branch '${branchName}' for pr #${prNumber}`,
    );
  } catch (error: any) {
    console.error(
      `failed to merge pr #${prNumber} (possibly conflicts or branch protection):`,
      error.message,
    );
...
async function markAsFailed(
  ...
  const reviewersToRequest = ["xhyrom", "nogodhenry"].filter(
    (user) => user !== authorLogin,
  );

  if (reviewersToRequest.length === 0) {
    console.log(
      `checks/merge failed for pr #${prNumber}, but authors cannot review their own pr. skipped requesting reviewers.`,
    );
    return;
  }
  ...
  try {
    ...
    console.log(
      `requested review from ${reviewersToRequest.join(", ")} for failed pr #${prNumber}`,
    );
  } catch (error: any) {
    console.error(
